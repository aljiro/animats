...
public:
    ...
    void init(double t0, double t1, int density){
        // Generates a point cloud centered at the origin

        // Area of the low dimensional projection
        ...
        // Distributing points in a curve in a surface of the sphere
        for(int i=0; i<N; i++){
            double area0 = area1;
            area1 += quad(res,T0[i],T1[i]);
            t = totalArea*((double)i/(double)(N-1));     // interpolation sample points
            u = (area0*(T1[i]-t)+area1*(t-T0[i]))/dt;    // linear interpolation
            v = (double)i*2.39996322972865332;       // fibonnacci golden angle
            X[i].resize(3);
            vector<double> f = func(u);
            // Parametrization of the sphere centered at the origin
            X[i][0] = f[0] * cos(v);
            X[i][1] = f[0] * sin(v);
            X[i][2] = f[1];
        }
...
class PSphere: public phyllotaxis {
